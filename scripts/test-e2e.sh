...
###############################################################################
# XENDA E2E Test — 10 wallets, full staking lifecycle on BSC mainnet
###############################################################################
...
TOKEN_ADDR="${XENDA_TOKEN_ADDR:?Set XENDA_TOKEN_ADDR from deploy output}"
STAKING_ADDR="${XENDA_STAKING_ADDR:?Set XENDA_STAKING_ADDR from deploy output}"

NUM_WALLETS=10
TOKENS_PER_WALLET="25000"
BNB_PER_WALLET="0.0003"
STAKE_AMOUNT="20000"
WALLETS_FILE="scripts/.test-wallets-$(date +%s).json"

# Helper: strip cast's "[1e25]" annotation and convert wei → human-readable
wei2eth() {
  local raw="$1"
  local clean
...
# Helper: strip annotation, return raw integer
raw_int() {
  echo "$1" | awk '{print $1}'
}
...
# ── Phase 0: Generate wallets ───────────────────────────────────────────────
echo ""
echo "▸ Phase 0: Generating $NUM_WALLETS wallets..."
...
for i in $(seq 0 $((NUM_WALLETS - 1))); do
  ...
  pk=$(echo "$output" | grep "Private key" | awk '{print $3}')
  ADDRS+=("$addr")
  PKS+=("$pk")
  if [ $i -gt 0 ]; then json+=","; fi
  json+="{\"id\":$i,\"address\":\"$addr\",\"pk\":\"$pk\"}"
  echo "   [$i] $addr"
done

json+="]"
echo "$json" > "$WALLETS_FILE"
echo "   Saved to $WALLETS_FILE"

# ── Phase 1: Fund wallets (BNB gas) ────────────────────────────────────────
echo ""
echo "▸ Phase 1: Sending $BNB_PER_WALLET BNB to each wallet..."

for i in $(seq 0 $((NUM_WALLETS - 1))); do
  $CAST send --rpc-url "$RPC" --private-key "$DEPLOYER_PK" \
    "${ADDRS[$i]}" --value "${BNB_PER_WALLET}ether" \
    --gas-limit 21000 > /dev/null 2>&1
  echo "   [$i] ${ADDRS[$i]} ← $BNB_PER_WALLET BNB ✓"
done
...
# Wallet 0 stakes with no referrer
...
# Wallets 1-3 → referrer W0 (Level 1)
for i in $(seq 1 3); do
  $CAST send --rpc-url "$RPC" --private-key "${PKS[$i]}" \
    "$STAKING_ADDR" "stake(uint256,uint256,address)" \
...
# Wallets 4-6 → referrer W1 (Level 2 for W0)
for i in $(seq 4 6); do
  $CAST send --rpc-url "$RPC" --private-key "${PKS[$i]}" \
    "$STAKING_ADDR" "stake(uint256,uint256,address)" \
...
# Wallets 7-9 → referrer W4 (Level 3 for W0)
for i in $(seq 7 9); do
  $CAST send --rpc-url "$RPC" --private-key "${PKS[$i]}" \
    "$STAKING_ADDR" "stake(uint256,uint256,address)" \
...
echo ""
echo "  Wallets saved: $WALLETS_FILE"
